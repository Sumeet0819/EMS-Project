...
import { RiMessage3Line, RiCloseLine, RiSendPlane2Line, RiArrowLeftLine, RiHashtag, RiAddLine, RiSearch2Line } from '@remixicon/react';
import { toast } from 'sonner';
import { useSocket } from '../../contexts/SocketContext';

// API utilities — no more inline axios calls
...
// Redux actions
import { setActiveChatUser, setMessages, addMessage, setConversations, incrementDmUnread, clearDmUnread, upsertConversation } from '../../store/reducers/messageSlice';
import { setChannels, addChannel, removeChannel, setActiveChannel, setChannelMessages, addChannelMessage, incrementChannelUnread } from '../../store/reducers/channelSlice';
...
const LiveChatNode = () => {
  ...
  const fetchData = useCallback(() => {
    ...
    Promise.all([
      ...
      const dmUnread = convs.reduce((a, c) => a + (c.unreadCount || 0), 0);
      const chUnread = chs.reduce((a, c)  => a + (c.unreadCount || 0), 0);
      setTotalUnread(dmUnread + chUnread);
    }).catch(() => {});
  ...
  useEffect(() => {
    ...
    const handleDM = (msg) => {
      const isActiveConversation = activeChatUser && msg.senderId === activeChatUser.id;
      if (isActiveConversation) {
        ...
        // If sender is a new partner we haven't seen before, refetch
        if (!sender) fetchData();

        toast.message(`New message from ${name}`, {
          description: msg.content.length > 40 ? msg.content.slice(0, 40) + '…' : msg.content,
          action: { label: 'Open', onClick: () => { setIsOpen(true); openDM(sender || { id: msg.senderId }); } }
        });
      }
    };
    ...
    const handleChannelMsg = (msg) => {
      if (activeChannel && msg.channelId === activeChannel.id) {
        ...
        toast.message(`New message in #${ch?.name || 'channel'}`, {
          description: msg.content.length > 40 ? msg.content.slice(0, 40) + '…' : msg.content,
          action: { label: 'Open', onClick: () => { setIsOpen(true); openChannel(ch); } }
        });
      }
    };
  ...
  const openDM = async (target) => {
    ...
    // Clear unread for this conversation in Redux + badge
  ...
  const openChannel = async (ch) => {
    ...
    if (ch.unreadCount > 0) setTotalUnread(prev => Math.max(0, prev - ch.unreadCount));
  ...
  const handleSend = async (e) => {
    ...
    if (activeChatUser) {
      try {
        ...
        // Ensure the partner is in conversations list
        dispatch(upsertConversation({
          id: activeChatUser.id,
          fullName: {
            firstName: activeChatUser.name.split(' ')[0],
            lastName: activeChatUser.name.split(' ').slice(1).join(' ')
          },
          role: 'employee',
          unreadCount: 0
        }));
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
      {isOpen && (
        <div className="mb-4 w-[360px] sm:w-[400px] h-[580px] max-h-[78vh] bg-card border border-border/60 shadow-2xl rounded-2xl overflow-hidden flex flex-col animate-in slide-in-from-bottom-5 duration-200">

          {/* Header */}
          <div className="flex items-center gap-2 px-4 py-3 bg-primary text-primary-foreground border-b border-primary/80 shrink-0">
            {inFocus ? (
              <>
                <button onClick={() => { inChat ? dispatch(setActiveChatUser(null)) : dispatch(setActiveChannel(null)); }} className="p-1 rounded hover:bg-white/10 transition-colors">
              ...
              <span className="font-semibold text-lg flex-1">Communications</span>
            )}
            <button onClick={() => setIsOpen(false)} className="ml-auto p-1 rounded hover:bg-white/10 transition-colors">
              <RiCloseLine size={20} />
            </button>
          </div>
            ...
            {!inFocus ? (
              <>
                ...
                <div className="px-3 py-2 border-b border-border/40 flex gap-2 bg-card shrink-0">
                  ...
                  {user.role === 'admin' && (
                    <Button size="sm" variant="ghost" onClick={() => setIsCreatingChannel(v => !v)} className="h-8 px-2 text-muted-foreground hover:text-foreground">
                      <RiAddLine size={16} />
                    </Button>
                  )}
                </div>
                ...
                <div className="overflow-y-auto flex-1 py-1">
                        ...
                        <div key={ch.id} onClick={() => openChannel(ch)} className="relative flex items-center gap-3 mx-2 px-3 py-2.5 rounded-xl hover:bg-muted/40 cursor-pointer transition-colors group">
                          ...
                          <div className="flex-1 overflow-hidden">
                            <div className="text-sm font-semibold truncate flex items-center gap-1.5">
                              {ch.name}
                              {ch.isBroadcast && <span className="text-[8px] px-1 py-px rounded bg-amber-500/10 text-amber-600 border border-amber-500/20 font-bold uppercase">Global</span>}
                            </div>
                  ...
                  {filteredChannels.length === 0 && filteredConvos.length === 0 && (
                    <div className="text-center text-xs text-muted-foreground py-16 px-6">
              ...
              <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-2.5">
                ...
                {(inChat ? messages : channelMessages).map((msg, i) => {
                  ...
                  return (
                    <div key={msg.id || i} className={`flex flex-col max-w-[82%] ${isMe ? 'self-end items-end' : 'self-start items-start'}`}>
                      {inChannel && !isMe && msg.sender && (
                        <span className="text-[9px] text-muted-foreground font-semibold ml-1 mb-0.5">
                          {msg.sender.firstName} {msg.sender.lastName}
                        </span>
                      )}
                      <div className={`px-4 py-2.5 rounded-2xl text-[13px] leading-snug shadow-sm ${isMe ? 'bg-primary text-primary-foreground rounded-br-sm' : 'bg-card border border-border/50 text-foreground rounded-bl-sm'}`}>
                        {msg.content}
                      </div>
      ...
      <Button
        ...
        className="relative h-14 w-14 rounded-full shadow-xl hover:shadow-2xl transition-all hover:-translate-y-1 bg-gradient-to-br from-primary to-primary/85 border-2 border-primary-foreground/10 text-primary-foreground group"
      >
        <div className="absolute inset-0 rounded-full bg-white opacity-0 group-hover:opacity-5 transition-opacity" />
        {isOpen ? <RiCloseLine size={26} /> : <RiMessage3Line size={26} />}
        {!isOpen && totalUnread > 0 && (
          <span className="absolute -top-1 -right-1 min-w-[20px] h-5 rounded-full bg-red-500 border-2 border-background flex items-center justify-center text-[10px] text-white font-black px-1 animate-bounce shadow-sm">
